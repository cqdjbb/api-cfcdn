...
zone_id = "xxxxxxxxxxx"
domain = "xxx.xxx"  # 您的二级域名
#域名列表
domains = ["1.xxx.xxx", "2..xxx.xxx" , "3..xxx.xxx", "4.xxx.xxx", "5.xxx.xxx"]
...
if response.status_code == 200:
    ...
    if code == 200:
        ...
        # 读取 CSV 文件并解析 IP 地址
        with open('/tmp/ip.csv', 'w', newline='', encoding='GB2312') as csvfile:
            ...
            for category, ips in info.items():
                for ip_info in ips:
                    ...
                    try:
                        ...
                        if category == "CU" :  # 筛选延迟不高于100的数据
                        #if delay <= 100 or (category == "CU" and delay <= 150) :  # 筛选延迟不高于100的数据
                            ...
                            response = requests.get(api_url, headers=headers, params={"name": domains[index]})
                            # 检查响应状态码
                            if response.status_code == 200:
                                # 解析 JSON 响应
                                result = response.json()
                            ...
                            # 检查响应状态码
                            if response.status_code == 200:
                                print(f"运营商:{category}, IP地址:{ip_info['ip']} 延迟:{delay}ms, 已成功解析到Cloudflare域名下")
                            else:
                                print(f"解析IP地址 {ip_info['ip']} 时出错：{response.text}")
                        else:
                            print(f"运营商:{category}, IP地址:{ip_info['ip']} 延迟:{delay}ms")
                    except ValueError:
                        print("无法转换延迟值为浮点数：{}".format(ip_info['delay']))

        print("最新优选IP已经保存 ip.csv 文件中")
    else:
